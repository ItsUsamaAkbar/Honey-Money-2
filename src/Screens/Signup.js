...
function SignUp(props) {
  ...
  const [secDocimage, setsecDocImage] = useState("");
  const imageRef = useRef(null);
  const IDimageRef = useRef(null);
  const DocimageRef = useRef(null);
  const secDocimageRef = useRef(null);
  ...
  const [secshowimage, setsecshowimage] = useState(false);
  ...
  const addUser = async () => {
    if (agreechecked !== true) {
      ...
      setEmailerror("You Can't left your Country,State and City Data Empty");
      setTimeout(() => {
        setEmailerror("");
      }, 6000);
    } else {
      ...
      createUserWithEmailAndPassword(auth, email, password)
        .then((per) => {
          ...
          setDoc(doc(db, "users", per.user.uid), {
            ...
            Image: result,
            IDImage: IDresult,
            DocumentImage: Docresult,
            EndDocumentImage: secDocresult,
  ...
  function Secdocresult() {
    const [secDocresult, setsecDocResult] = useState("");

    function secDocuploader(e) {
      const imageFile = e.target.files[0];

      const reader = new FileReader();
      reader.addEventListener("load", (e) => {
        setsecDocResult(e.target.result);
      });

      reader.readAsDataURL(imageFile);
    }

    return { secDocresult, secDocuploader };
  }
  const { secDocresult, secDocuploader } = Secdocresult();

  //====================================================
  //====================================================
  function useDocDisplayImage() {
    const [Docresult, setDocResult] = useState("");

    function Docuploader(e) {
      const imageFile = e.target.files[0];

      const reader = new FileReader();
      reader.addEventListener("load", (e) => {
        setDocResult(e.target.result);
      });

      reader.readAsDataURL(imageFile);
    }

    return { Docresult, Docuploader };
  }
  const { Docresult, Docuploader } = useDocDisplayImage();
            ...
            <Form className="forminput radius">
              ...
              <div className="border p-3">
                <Row>
                  <Col>
                    <p>
                      A complete profile will bump you up in Search Results and
                      is recommended
                    </p>
                  </Col>
                </Row>
                    ...
                    <Row>
                      <Col style={{ width: "235px" }}>
                        ...
                        >
                          ...
                          <p className="btntxt">
                            Upload Government ID (like Drivers License,
                            Passport, Company Registration)
                          </p>
                        </Button>
                      </Col>
                    </Row>
                            ...
                            <Row>
                              <Col style={{ width: "20vh" }}>
                                ...
                                >
                                  ...
                                  <p className="btntxt">Upload Documents</p>
                                </Button>
                              </Col>
                            </Row>
                          </Col>
                            ...
                            {secshowimage === false ? (
                              ...
                              <Col style={{ width: "20vh" }}>
                                <input
                                  ...
                                  type="file"
                                  onChange={(e) => {
                                    setsecDocImage(e.target.files[0]);
                                    secDocuploader(e);
                                  }}
                                />
                                <Button
